...
int NbElm(List2 L) {
    ...
    if (!IsEmptyList(L)) { // jika list empty
        ...
        do {
            n++;
            P = next(P);
        } while (P != First(L)); // end while jika p = first(L)
    }
    return n;
}
    ...
    if (P != NIL) { // jika P adalah kosong
        if (IsEmptyList(*L)) { // jika list kosong
            ...
            while (next(Last) != First(*L)) {
                Last = next(Last);
            } // end while ketika elemen selanjutnya dari elemen last adalah elemen pertama dari list
            next(P) = First(*L);
            next(Last) = P;
            First(*L) = P;
        }
...
void DeleteVFirst(List2 *L, infotype *V) {
    ...
    if (!IsEmptyList(*L)) { // Jika list tidak kosong
        P = First(*L);
        *V = info(P);
        if (IsOneElm(*L)) { // Jika list cuma 1 elemen
            First(*L) = NIL;
    ...
    *V = '#';
    if (!IsEmptyList(*L)) { // jika list tidak kosong
        ...
        while (next(P) != First(*L)) {
            Prec = P;
            P = next(P);
        } // end while ketika elemen selanjutnya dari p adalah elemen pertama dari list
        *V = info(P);
        if (Prec == NIL) { // jika prec kosong
            First(*L) = NIL;
        } else { // jika prec tidak kosong
            next(Prec) = First(*L);
        }
        Dealokasi(&P);
    }
...
void DeleteX(List2 *L, infotype X) {
    ...
    if (!IsEmptyList(*L)) { // jika list tidak empty
        ...
        Prec = Last;
        do {
            if (info(P) == X) { // jika hasilnya sama
                if (P == First(*L)) { // jika p adalah elemen pertama list
                    DeleteVFirst(L, &X);
                } else if (next(P) == First(*L)) { // jika elemen selanjutnya dari p adalah elemen pertama list
                    DeleteVLast(L, &X);
                } else { 
                    next(Prec) = next(P);
                    Dealokasi(&P);
            ...
            Prec = P;
            P = next(P);
        } while (P != First(*L)); // end while ketika p = first(*L)
    }
}
...
void Invers(List2 *L) {
    //kamus lokal
    address sebelum, sekarang, Next, Last;
    //algoritma
    if (!IsEmptyList(*L) && !IsOneElm(*L)) { // jika list tidak empty dan lebih dari satu elemen
        Last = First(*L);
        while (next(Last) != First(*L)) {
            Last = next(Last);
        } // end while ketika next(Last) = First(*L)
        sebelum = Last;
        sekarang = First(*L);
        do {
            Next = next(sekarang);
            ...
            sebelum = sekarang;
            sekarang = Next;
        } while (sekarang != First(*L)); // end while ketika sekarang = first(*L)
        First(*L) = sebelum;
    }
}
...
int CountX(List2 L, infotype X) {
    ...
    //algoritma
    count = 0;
    if (!IsEmptyList(L)) { // jika tidak isEmptyList(L)
        P = First(L);
        do {
            if (info(P) == X) count++;
            P = next(P);
...
void InsertVBeforeX(List2 *L, infotype X, infotype V) {
    ...
    if (!IsEmptyList(*L)) {
        ...
        Prec = NIL;
        do {
            if (info(P) == X) { // Jika elemen dari memori P adalah X
                if (Prec == NIL) { // Jika prec = kosong
                    InsertVFirst(L, V);
                } else { // Jika prec tidak kosong
                    New = Alokasi(V);
...
void DeleteVAfterX(List2 *L, infotype X, infotype *V) {
    ...
    *V = '#';
    if (!IsEmptyList(*L)) {
        P = First(*L);
        do {
            if (info(P) == X && next(P) != P) { // jika info(P) == X dan next(P) tidak sama dengan P
                Del = next(P);
                *V = info(Del);
                next(P) = next(Del);
                Dealokasi(&Del);
...
void DeleteVBeforeX(List2 *L, infotype X, infotype *V) {
    ...
    *V = '#';
    if (!IsEmptyList(*L)) {
        ...
        Prec2 = NIL;
        do {
            if (info(P) == X && Prec != NIL) { // Jika info(P) == X && Prec != NIL
                *V = info(Prec);
                if (Prec2 == NIL) { // Jika Prec2 kosong
                    DeleteVFirst(L, V);
                } else {
                    next(Prec2) = P;
                    Dealokasi(&Prec);
...
void DeleteAllX(List2 *L, infotype X) {
    ...
    while (CountX(*L, X) > 0) {
        DeleteX(L, X);
    } // end while ketika hasil countX = 0
}
...
char Modus(List2 L) {
    ...
    do {
        c = CountX(L, info(P));
        if (c > max) { // jika c lebih dari max
            max = c;
            m = info(P);
        }
        P = next(P);
...
void ConcatList(List2 L1, List2 L2, List2 *L) {
    ...
    //algoritma
    CreateList(L);
    if (!IsEmptyList(L1)) { // Jika list1 tidak empty
        P = First(L1);
        do {
            InsertVLast(L, info(P));
            P = next(P);
        } while (P != First(L1)); // end while ketika P = First(L1)
    }
    if (!IsEmptyList(L2)) { // Jika list2 tidak empty
        Q = First(L2);
        do {
            InsertVLast(L, info(Q));
            Q = next(Q);
        } while (Q != First(L2)); // end while ketika Q = First(L2)
    }
}
...
void SplitList(List2 L, List2 *L1, List2 *L2) {
    ...
    for (i = 1; i <= mid; i++) {
        ...
        P = next(P);
    } // end interval jika i kurang dari sama dengan mid
    for (i = mid + 1; i <= n; i++) {
        InsertVLast(L2, info(P));
        P = next(P);
    } // end interval jika i kurang dari sama dengan n
}
...
void CopyList(List2 L1, List2 *L2) {
    ...
    //algoritma
    CreateList(L2);
    if (!IsEmptyList(L1)) { // Jika list 1 tidak empty
        P = First(L1);
        do {
            InsertVLast(L2, info(P));
            P = next(P);
