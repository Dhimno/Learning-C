...
// /* procedure PrintLevel(input P:bintree, input N:integer) {Di comment karena double dari pohon1.c}
...
void AddDaunTerkiri(bintree *P, infotype X) {
    ...
    if (*P != NIL) {
        curr = *P;
        while (left(curr) != NIL) curr = left(curr);
        newNode = AlokasiTree(X);
        if (newNode != NIL) left(curr) = newNode;
    }
}
